...
# Replace ";,;" with another character so we can split then by ","
daily_sales_replaced = daily_sales.replace(";,;", "__")

# Separate the string into a list with each transaction
daily_transactions = daily_sales_replaced.split(",")

# Split each individual transaction into a list of its data points
daily_transactions_split = []
for item in daily_transactions:
  daily_transactions_split.append(item.split("__"))
...
for txn in daily_transactions_split:
  ...
  for item in txn:
    transactions_clean_2. append(item.strip())
  transactions_clean.append(transactions_clean_2)

# Collect individual data points for each transaction
customers = []
sales = []
thread_sold = []
...
# how much sales are made in a day
total_sales = 0
for sale in sales:
  sale = sale.replace("$", "")
  sale = float(sale)
...
# Count the number of times each color was sold
...
# Define a list of the color offered
colors = ['red', 'yellow', 'green', 'white', 'black', 'blue', 'purple']

# Report how many threads of each color were sold today
index = 0
for color in colors:
